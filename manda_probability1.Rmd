...
When there is 1 Streak of it means one hit in a row. When there is a Length of 0, that would mean there was a miss. No streak basically
...
From looking at the barpolt you can tell that Kobe's steak has a right skew. His typical steak was about 0-1. And his longest steak was 4 hits.
...
```{r}
sim_streak <- calc_streak(sim_basket1)
...
```{r}
cat("Longest streak is:", max(sim_streak))
```
...
cat("Longest Streak is:", max(sim_streak2))
```

After running the simulation again, it seems my hypothesis was correct - most streaks were around 0-1, with a few high streaks.
...
Based off the analysis, Kobe Bryant's distrubution of his streak lengths compares to the simulated shooter. They both on average have mostly 0-1 hits for their streak.  Analyzing  at the data, I don't believe there is enough evidence to say the hot hand model fits.
