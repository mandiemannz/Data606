...
#The data appears to be skewed right. The typical size in the sample is close to the original population of 1499. Typical meaning a value near the most concurrent values in the distrubtion.


2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

#  I wouldn't expect another students to be identical to mine, since this is a random sample.
...
# For the conditions to be true, the sameple must consist of 30 independent observations and shouldn't be skewed too much. 
...
#95% confidence mean referrs to the confidence interval level of the normal model with a standard error. 
...
#Certainly, 1499 is between our interval.
...
#95%  can capture the population mean
...
```{r} 
low_vector99 <- samp_mean - 2.58 * samp_sd / sqrt(n) 
up_vector99 <- samp_mean + 2.58 * samp_sd / sqrt(n)
plot_ci(low_vector99, up_vector99, mean(population))
```
```{r}
v <- data.frame(lower_vector, upper_vector)
pop_mean <- mean(population)

left <- sum(v$upper_vector < pop_mean)
right <- sum(v$lower_vector > pop_mean)
