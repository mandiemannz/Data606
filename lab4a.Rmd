...
##This population distribution is unimodal, slightly skewed to the right and has a few outliers.The mean is 1500.
...
##sample_means50 has 5000 total elements, which are taken from mean calculations for each sample. The same distribution takes a normal distribution.
 
 ##A sample of 50,000 would make an even better approximation.
...
```
#The vector's elements represent a sample mean of 50 random variables from the area vector.
...
#When the sample size is larger, the center becomes more near the real population mean and spread becomes smaller
...
#The best estimate of population mean is 178304.7
...
-   Since you have access to the population, simulate the sampling 
    ...
    ```
#The shape of the distribution appear normal, centered around 180k
...
```
##The bigger the sample, the more the data is centered around the mean, with less spread
...
##The sample with 150 has smaller spread. smaller spread.
