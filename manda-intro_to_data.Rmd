---
title: "Introduction to data, Lab 1"
author: Amanda Arce
output:
  html_document:
    theme: cerulean
...
# Basic scatter plot
ggplot( cdc, aes(cdc$weight, cdc$wtdesire)) + geom_point(aes(color = gender))
...
```
 What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    class(wdiff)
```
If wdiff is  at '0', that will mean the person currently at his/her desired weight.

If wdiff is at < '0', then the person knows that in order to reach their goal they will have to drop weight.

If wdiff is > '0', then the person knows that in order to reach their goal they will need to increase their weight.
...
```{r}
hist(wdiff, main="Difference Weight Distribution",
   xlab="Weight diff") 
...
weightSd <- sd(cdc$weight)

weighTtotaladd <- weightMean + weightSd 
weighTtotalsub <- weightMean - weightSd 

total <- pnorm(weighTtotaladd, weightMean, weightSd , lower.tail = T) - pnorm(weighTtotalsub, weightMean, weightSd , lower.tail = T)
