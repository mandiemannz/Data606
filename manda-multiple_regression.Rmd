...
1.  Is this an observational study or an experiment? The original research
    ...
    It is an observation study. A rephrased question could be: "is beauty correlated with differences in course evaluation"
...
The distrubtuion is skewed to the left. The center is around 4-5. Most students gave the courses a high rating.
...
score <- .06664 * evals$bty_avg + 3.88034
...
The model predicts that non-minority professors will score ~.12 higher.
...
model1 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval + cls_students + cls_level  + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(model1)
```
...
```{r}
best <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval + cls_credits + bty_avg + pic_color, data = evals)
summary(best)
```
...
The model predicts: - score for males will be .21 higher, professors from non-engolish schools will be .21 lower. - color photos .19 lower, non-minorities will be .17 higher - one year increase in age the score will be .01 lower

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?
    
No. Linear model conditions haven't been satasifed.
