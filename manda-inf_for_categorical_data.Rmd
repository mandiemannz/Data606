...
From reading though the docuement, I can say the data is independent, and randomly selected. We can also assume the data is largle sample. 
## The data
...
The rows correspond to the percentages in religion. 
...
```
5% are atheist from looking at the table.
...
Finland <- subset(atheism, nationality == "Finland" & year == "2012")

inference(Finland$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
...
Aus.p <- 0.1
Aus.np <- Aus.n*Aus.p
paste("Australia np: ", Aus.np)


Aus.n1p <- Aus.n*(1-Aus.p)
paste("Australia n(1-p): ", Aus.n1p)

# For Ecuador
Ecu.n <- 400
Ecu.p <- 0.02
Ecu.np <- Ecu.n*Ecu.p
paste("Ecuador np: ", Ecu.np)




Ecu.n1p <- Ecu.n*(1-Ecu.p)
paste("Ecuador n(1-p): ", Ecu.n1p)
```
...
H0 There is no convincing evidence that Spain has seen a change in its atheism between 2005 and 2012

Ha There is convincing evidence that Spain has seen a change in its atheism between 2005 and 2012

```{r}
spain12 <- subset(atheism, nationality == "Spain" & year == "2005"  | nationality == "Spain" & year == "2012")

inference(y = spain12$response, x = spain12$year, est = "proportion",type = "ht", null = 0, alternative = "twosided", method = "theoretical", success = "atheist")
...
H0 There is no convincing evidence that US  has seen a change in its atheism between 2005 and 2012

Ha There is convincing evidence that US has seen a change in its atheism between 2005 and 2012
...
There are 39 countires listed in table 4. 0.05 * 39 = 1.95 countires we would expect to detect a change by chance.
