...
The plots from the simulated data look similar to the fdims$hgt plot. The data seems to mostly follow closly to the line, with some of the data away by the tails.   
...
The weight data seems to be further from the line than the height data. There are also some outliers far from the line. A histogram also shows somewhat of a right skew but still closly normal.
...
-   Now let's consider some of the other variables in the body dimensions data 
    ...
    **d.** The histogram for female chest depth (`che.de`) belongs to normal 
    probability plot letter A____.

-   Note that normal probability plots C and D have a slight stepwise pattern.  
    Why do you think this is the case?
    
    The variables dont seem as continous as the other ones.
